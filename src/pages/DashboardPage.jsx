  ...
  return (
    <svg width={size} height={size} style={{ flexShrink: 0 }}>
      ...
      <text x={size/2} y={size/2 + 1} textAnchor="middle" dominantBaseline="middle"
        fill={color} fontSize="11" fontWeight="700" fontFamily="Syne, sans-serif">
        {pct}%
      </text>
    </svg>
  )
  ...
  return (
    <div style={{ color: v('text1'), fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px', marginBottom: '20px' }}>
        ...
        {/* Completion rate */}
        <div style={{ ...card(), background: 'linear-gradient(135deg,#1a1a2e,#16213e)', border: '1px solid rgba(107,127,255,0.25)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#6b7fff', marginBottom: '12px' }}>Completion Rate</div>
        ...
        {/* Completed this week */}
        <div style={{ ...card(), background: 'linear-gradient(135deg,#0d2818,#0a2010)', border: '1px solid rgba(78,203,131,0.25)', position: 'relative', overflow: 'hidden' }}>
        ...
        {/* Overdue */}
        <div style={{ ...card(), background: stats.overdue > 0 ? 'linear-gradient(135deg,#2e0d0d,#200a0a)' : card().background, border: `1px solid ${stats.overdue > 0 ? 'rgba(255,95,109,0.25)' : v('border')}`, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ fontSize: '12px', color: v('text3') }}>{stats.overdue === 0 ? 'All on track ✓' : 'tasks need attention'}</div>
        </div>

        {/* High priority */}
        <div style={{ ...card(), background: 'linear-gradient(135deg,#2a1a10,#1e1208)', border: '1px solid rgba(232,160,74,0.25)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ ...card(), marginBottom: '20px' }}>
        <div style={{ fontSize: '13px', fontWeight: 600, color: v('text2'), textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '18px' }}>
          Task distribution
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '20px' }}>
        ...
        <div style={card()}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
            ...
            <button
              onClick={() => onNavigate('Board')}
              style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '12px', color: '#e8a04a', fontFamily: 'DM Sans, sans-serif', fontWeight: 500 }}
            >
              View all →
            </button>
          </div>
            ...
            {stats.recentTasks.length === 0 ? (
              ...
              return (
                <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 0', borderBottom: `1px solid ${v('border')}` }}>
                  ...
                  <span style={{ flex: 1, fontSize: '13px', color: t.status === 'completed' ? v('text3') : v('text1'), textDecoration: t.status === 'completed' ? 'line-through' : 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {t.title}
                  </span>
                  <span style={{ fontSize: '10px', padding: '2px 7px', borderRadius: '20px', background: pm.bg, color: pm.color, fontWeight: 600, flexShrink: 0 }}>
                    {pm.label}
                  </span>
                </div>
              )
          ...
          {stats.dueSoon.length === 0 ? (
            ...
            return (
              <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 0', borderBottom: `1px solid ${v('border')}` }}>
                ...
                <span style={{ flex: 1, fontSize: '13px', color: v('text1'), overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {t.title}
                </span>
                <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '20px', fontWeight: 600, flexShrink: 0,
                  background: isToday ? 'rgba(255,95,109,0.15)' : 'rgba(232,160,74,0.15)',
                  color: isToday ? '#ff5f6d' : '#e8a04a' }}>
      ...
      {/* ── QUICK ACTIONS ── */}
      <div style={card()}>
        <div style={{ fontSize: '13px', fontWeight: 600, color: v('text2'), textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '16px' }}>
          Quick actions
        </div>
